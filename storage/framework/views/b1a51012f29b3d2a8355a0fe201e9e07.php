    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
        <form method="GET" class="flex flex-1 gap-2">
            <input type="text" name="q" value="<?php echo e(request('q')); ?>" placeholder="Cari lokasi warung / alamat..."
                   class="w-full sm:w-72 rounded-xl border-stone-300 text-sm focus:ring-amber-500 focus:border-amber-500">
            <select name="status" class="rounded-xl border-stone-300 text-sm focus:ring-amber-500 focus:border-amber-500">
            ...
            <button class="px-4 py-2 rounded-xl bg-stone-800 text-white text-sm font-medium">Cari</button>
        </form>

        <a href="<?php echo e(route('titipan.create')); ?>" class="inline-flex items-center justify-center px-4 py-2.5 rounded-xl bg-amber-500 text-[#241C19] font-semibold text-sm hover:bg-amber-400 transition whitespace-nowrap">
            + Buat Titipan
        </a>
    </div>
            ...
            <tbody class="divide-y divide-stone-100">
                <?php $__empty_1 = true; $__currentLoopData = $titipans; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $t): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <tr class="hover:bg-stone-50">
                        <td class="px-5 py-3">
                            <p class="font-medium text-stone-800"><?php echo e($t->lokasi_warung); ?></p>
                            <p class="text-xs text-stone-400"><?php echo e($t->alamat_antar); ?></p>
                        </td>
                        <td class="px-5 py-3 text-stone-600"><?php echo e($t->items->pluck('nama_item')->join(', ')); ?></td>
                        <td class="px-5 py-3 text-stone-600"><?php echo e($t->driver?->name ?? '-'); ?></td>
...
<?php endif; ?></td>
                        <td class="px-5 py-3 text-stone-400"><?php echo e($t->created_at->diffForHumans()); ?></td>
                        <td class="px-5 py-3 text-right space-x-2 whitespace-nowrap">
                            <a href="<?php echo e(route('titipan.show', $t)); ?>" class="text-amber-700 hover:underline">Detail</a>
                ...
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                    <tr><td colspan="6" class="px-5 py-8 text-center text-stone-400">Belum ada titipan. Yuk buat titipan pertama!</td></tr>
                <?php endif; ?>
            </tbody>
        </table>
    </div>
    ...
    <div x-data="{ show: false, id: null }" @open-batal.window="show = true; id = $event.detail.id"
        ...
        <div @click.outside="show = false" class="bg-white rounded-2xl p-6 w-full max-w-sm">
            <h3 class="font-semibold text-stone-800 mb-3">Batalkan Titipan</h3>
            <form :action="'/titipan/' + id" method="POST">
                <?php echo csrf_field(); ?> <?php echo method_field('DELETE'); ?>
                <label class="block text-sm text-stone-600 mb-1">Alasan pembatalan</label>
                <textarea name="alasan_batal" required rows="3" class="w-full rounded-xl border-stone-300 text-sm focus:ring-amber-500 focus:border-amber-500"></textarea>
                <div class="flex justify-end gap-2 mt-4">
                    <button type="button" @click="show = false" class="px-4 py-2 rounded-xl text-sm text-stone-600">Batal</button>
