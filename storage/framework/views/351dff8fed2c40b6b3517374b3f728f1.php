...
<?php $component->withAttributes([]); ?>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6 bg-white rounded-2xl border border-slate-200 p-5 shadow-sm transition-all duration-300 hover:shadow-lg hover:-translate-y-1">
        ...
        <form method="POST" action="<?php echo e(route('driver.toggle')); ?>">
            <?php echo csrf_field(); ?>
            <button class="px-5 py-2.5 rounded-xl font-semibold text-sm transition-all duration-300 ease-in-out hover:scale-105 active:scale-95 <?php echo e($user->is_driver_active ? 'bg-slate-800 text-white hover:bg-slate-700' : 'bg-blue-500 text-[#0f172a] hover:bg-blue-400'); ?>">
            ...
            <?php $__currentLoopData = $sedangDiantar; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $t): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <div class="bg-white rounded-2xl border border-blue-200 p-5 shadow-sm transition-all duration-300 hover:shadow-lg hover:-translate-y-1">
                    <div class="flex items-start justify-between mb-2">
                        <div>
                            <p class="font-medium text-slate-800"><?php echo e($t->lokasi_warung); ?> &rarr; <?php echo e($t->alamat_antar); ?></p>
                            <p class="text-xs text-slate-400">Pembeli: <?php echo e($t->pembeli?->name); ?> &middot; <?php echo e($t->items->pluck('nama_item')->join(', ')); ?></p>
                        </div>
    ...
    <div class="grid gap-4" x-data
        ...
        <?php $__empty_1 = true; $__currentLoopData = $titipans; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $t): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
            <div class="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm transition-all duration-300 hover:shadow-lg hover:-translate-y-1 flex items-start justify-between gap-4">
                <div>
                    <p class="font-medium text-slate-800"><?php echo e($t->lokasi_warung); ?> &rarr; <?php echo e($t->alamat_antar); ?></p>
                    <p class="text-xs text-slate-400 mt-1"><?php echo e($t->items->pluck('nama_item')->join(', ')); ?></p>
                    <p class="text-xs text-slate-400">Ongkir: Rp <?php echo e(number_format($t->ongkir,0,',','.')); ?> &middot; Bayar: <?php echo e(strtoupper($t->metode_bayar)); ?></p>
                    <?php if(isset($t->jarak_km)): ?>
                        <p class="text-xs font-medium text-blue-700 mt-1">&asymp; <?php echo e($t->jarak_km); ?> km dari lokasimu</p>
                    <?php endif; ?>
                </div>
                <form method="POST" action="<?php echo e(route('driver.ambil', $t)); ?>">
                    <?php echo csrf_field(); ?>
                    <button class="px-4 py-2 rounded-xl bg-blue-500 text-[#0f172a] text-sm font-semibold whitespace-nowrap hover:bg-blue-400" <?php if(!$user->is_driver_active): ?> disabled <?php endif; ?>>
                        Ambil Order
                    </button>
                </form>
            </div>
